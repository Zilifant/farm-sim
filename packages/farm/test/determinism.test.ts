...
import {
  CORN, FARM_BORROW, FARM_BUILD_ROAD, FARM_CREATE_FIELD, FARM_SCHEDULE_OP,
  FARM_SELL, OP_FERTILIZE, OP_HARVEST, OP_PLANT,
...
async function scriptedRun(seed: number | string): Promise<FarmSim> {
  ...
  at(2, { kind: FARM_BUILD_ROAD, cells: Array.from({ length: 27 }, (_, y) => ({ x: 34, y })) });
  at(3, { kind: FARM_CREATE_FIELD, x: 24, y: 0, w: 10, h: 9 });
  at(3, { kind: FARM_CREATE_FIELD, x: 24, y: 9, w: 10, h: 9 });
...
describe("determinism", () => {
  ...
  it("a recorded command log reproduces the run", async () => {
    ...
    original.apply({ kind: FARM_BUILD_ROAD, cells: Array.from({ length: 27 }, (_, y) => ({ x: 34, y })) });
